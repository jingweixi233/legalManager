...
def nerual_network(input_train,output_train,reasons):
	...
	history = model.fit(input_train,output_train,validation_split=0.15,batch_size=500,epochs=20,callbacks=[early_stopping])
...
def get_result(input_train,output_train,input_test,output_test,output_test_c,reasons,reasonNum):
	f=open('result.txt','a')
	model = load_model('model for criminal 95.h5')
